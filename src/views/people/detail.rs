...
pub fn view_cluster_detail(
    ...
    let photo_ids_for_album: Vec<i64> = photos.iter().map(|p| p.id).collect();
    let add_album_btn = button(text("Add all to Album").size(12).color(text_secondary))
        .padding(Padding::from([6, 12]))
        .style(move |_theme: &iced::Theme, status: button::Status| {
            let background = match status {
                button::Status::Hovered => Some(bg_hover.into()),
        ...
        .on_press(Message::OpenAlbumPicker(photo_ids_for_album));
